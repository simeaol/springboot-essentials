...
public class WriteCardRepositoryImpl implements WriteCardRepository {
    ...
    @Override
    public void findCardById(UUID externalId) {
        jdbcTemplate.queryForObject("select id, external_id, name from card where external_id = : _externalId",
                new MapSqlParameterSource().addValue("_externalId", externalId), Card.class);
    }
}
